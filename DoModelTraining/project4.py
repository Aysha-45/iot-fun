...
def load_data(file_path):
    ...
    data['time'] = pd.to_datetime(data['time'], format='%d/%m/%Y %H:%M').astype(np.int64) // 10**9
    return data
...
# Inside train_and_save_models function
    ...
    X = data.drop(['pressure', 'temp', 'humid', 'time'], axis=1)
    data['time_processed'] = data['time']
    X['time_processed'] = data['time_processed']
    y = data[['pressure', 'temp', 'humid']]
