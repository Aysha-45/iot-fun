...
svm_model1 = joblib.load('../DoModelTraining/models/temp_SVM.pkl')  # Load temperature SVM model
svm_columns1 = joblib.load("../DoModelTraining/models/temp_LinearRegression.pkl")  # Load temperature model columns
svm_model2 = joblib.load('../DoModelTraining/models/humid_SVM.pkl')  # Load humidity SVM model
svm_columns2 = joblib.load("../DoModelTraining/models/humid_LinearRegression.pkl")  # Load humidity model columns
...
@app.route('/')
def check_model():
    if svm_model1 is not None and svm_model2 is not None:
        return "Models are ready for prediction"
    return "Server is running but something is wrong with the models"
...
def predict():
    try:
        ...
        # Return both predicted temperature and humidity in JSON format
        return jsonify({
            "Predicted temperature": predicted_output1,
            "Predicted humidity": predicted_output2
        }), 200
